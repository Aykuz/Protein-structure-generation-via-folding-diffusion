...
def images_to_gif(
    images: Collection[str], fname: str, pause_on_last: bool = True, loop: bool = False
) -> str:
    """Create a gif from the given images, returns output filename"""
    # https://stackoverflow.com/questions/753190/programmatically-generate-video-or-animated-gif-in-python
    images_read = [imageio.v2.imread(f) for f in images]
    # Show each frame for 0.05 seconds, and the last one for 10 seconds
    frametimes = [0.02] * len(images)
    if pause_on_last:
        frametimes[-1] = 10.0
    imageio.mimsave(
        fname,
...
def images_to_gif_from_args(args):
    ...
    # sorted_inputs = sorted(input_files, key=get_int_tuple)
